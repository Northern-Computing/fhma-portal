...
def get_or_create_surveyor(name):
    response = requests.get(SURVEYOR_API_URL, params={'search': name}, auth=HTTPBasicAuth(USERNAME, PASSWORD))
...
def get_or_create_zipcode(zipcode):
    response = requests.get(ZIPCODE_SURVEY_API_URL, params={'search': zipcode}, auth=HTTPBasicAuth(USERNAME, PASSWORD))
    if response.status_code == 200 and response.json():
        return response.json()[0]['id']
    else:
...
def ingest_data():
    ...
    with open(CSV_FILE_PATH, newline='') as csvfile:
        ...
        for row in reader:
            ...
            surveyor_id = get_or_create_surveyor(surveyor_name)
            ...
            if surveyor_id and zipcode_id:
                client_survey_data = {
                    ...
                    'medical_treatment': json.dumps(row['If you sought medical care, did you have to see a doctor or go to the ER?'].split(', ')),
                    ...
                    'helped_leave_home': json.dumps(row['Has having incontinent products helped you to leave home more to:'].split(', ')),
                    'zipcode': zipcode_id,
